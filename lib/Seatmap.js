...
var _immutableDistImmutableMinJs2 = _interopRequireDefault(
  _immutableDistImmutableMinJs
);
...
var Seatmap = (function(_React$Component) {
  function Seatmap(props) {
    ...
    this.selectSeat = function(row, number) {
      ...
      var seatAlreadySelected = selectedSeats
        .get(row, _immutableDistImmutableMinJs.Set())
        .includes(number);
        ...
        _this.setState(
          {
            selectedSeats: selectedSeats.mergeDeep(
              ((_selectedSeats$mergeDeep = {}),
              (_selectedSeats$mergeDeep[row] = _immutableDistImmutableMinJs.Set(
                [number]
              )),
              _selectedSeats$mergeDeep)
            ),
  ...
  Seatmap.prototype.shouldComponentUpdate = function shouldComponentUpdate(
    nextProps,
    nextState
  ) {
  ...
  Seatmap.prototype.renderRows = function renderRows() {
    ...
    return this.props.rows.map(function(row, index) {
      var rowNumber = alpha
        ...
        : (index + 1).toString();
      var isSelected = !seats
        .get(rowNumber, _immutableDistImmutableMinJs.Set())
        .isEmpty();
      var props = {
        rowNumber: rowNumber,
  ...
  Seatmap.prototype.renderSeats = function renderSeats(
    seats,
    rowNumber,
    isRowSelected
  ) {
    ...
    return seats.map(function(seat, index) {
      if (seat === null)
        return _react2["default"].createElement(_Blank2["default"], {
          key: index
        });
      var isSelected =
